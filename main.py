    ...
    def __crear_arcos(self,indice_vertice):
        if self.grafo.vcount()!=0: #si el grafo no esta vacío
            ...
            for vs in vertices_similares:
                x=len(self.grafo.es.select(_within=[vs.index,indice_vertice]))
                if (len(self.grafo.es.select(_within=[vs.index,indice_vertice]))<2):
                    if(vs["Codigo"]!=self.grafo.vs[indice_vertice]["Codigo"]):
    ...
    def buscar_nodos(self, nombre):
        ...
        while cola:
            ...
            if nodo_actual not in visitados:
                ...
                if self.grafo.vs[nodo_actual]["name"] == nombre:
                    print(f"Encontrado nodo con nombre '{nombre}' en el índice {nodo_actual}")
                    return nodo_actual
                for vecino in self.grafo.neighbors(nodo_actual):
                    if vecino not in visitados:
                        cola.append(vecino)

        print(f"Nodo con nombre '{nombre}' no encontrado")
        return None
